...
@endsection
    ...
    @section('content')
        ...
        @include('admin.templates.products.list')


        @include('admin.templates.products.add')
        @include('admin.templates.products.update')
...
{
  var API_URL = "{{ route('admin.products.list') }}";
    $.ajax({
     url: API_URL,
     method: 'post',
     data: {},
...
function add_row(form)
{
    ...
    formData.append('image_second', $('input[type=file]')[0].files[0]);
    formData.append('image_third', $('input[type=file]')[0].files[0]);

     var API_URL = "{{ route('admin.products.store') }}";
    ...
    function get_row(id)
    {
     var API_URL = "{{route('admin.products.get')}}";
        $.ajax({
            url: API_URL,
            type: 'POST',
           data: {'id': id,'_token': '{!! csrf_token() !!}'},
            ...
            success: function(data)
            {
              if (data["status"] == 1){
                 ...
                 $('#update_category_id').val(data["data"]["category_id"]);
                 $('#update_price').val(data["data"]["price"]);
                 $('#update_bulk_price').val(data["data"]["bulk_price"]);
                 $('#update_min_quantity').val(data["data"]["min_quantity"]);
                 $('#update_b2b_Price').val(data["data"]["b2b_Price"]);
                ...
                if(data["data"]["image_second"] != null){
                    update_readURLImage(data["data"]["image_second"],'update_image_second_preview')
                }
                if(data["data"]["image_third"] != null){
                    update_readURLImage(data["data"]["image_third"],'update_image_third_preview')
                }
