...
@endsection
    ...
    @section('content')
        ...
        @include('admin.templates.homepagebanners.list')


        @include('admin.templates.homepagebanners.add')
        @include('admin.templates.homepagebanners.update')
...
{
  var API_URL = "{{ route('admin.homepagebanners.list') }}";
    $.ajax({
     url: API_URL,
     method: 'post',
     data: {},
...
function add_row(form)
{
    ...
    formData.append('banner_image', $('input[type=file]')[0].files[0]);
 
    
     var API_URL = "{{ route('admin.homepagebanners.store') }}";
    ...
    function get_row(id)
    {
     var API_URL = "{{route('admin.homepagebanners.get')}}";
        $.ajax({
            url: API_URL,
            type: 'POST',
           data: {'id': id,'_token': '{!! csrf_token() !!}'},
            ...
            success: function(data) 
            {
                ...
                update_readURLImage(data["data"]["image"],'update_image_preview') 
                } 
     ...
     function update_row()
     {
         ...
         var API_URL = "{{ route('admin.homepagebanners.update') }}";
            $.ajax({
                url: API_URL,
                 type: 'POST',
                 data: formData,
    ...
    if(t.value == true){ 
        var API_URL = "{{ route('admin.homepagebanners.delete') }}";
        $.ajax({
            type: "POST",
            url: API_URL,
